...
#define GO(A) \
    if (my_dl_iterate_phdr_fct_##A == (uintptr_t)fct) return my_dl_iterate_phdr_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_dl_iterate_phdr_fct_##A == 0) {           \
        my_dl_iterate_phdr_fct_##A = (uintptr_t)fct; \
        return my_dl_iterate_phdr_##A;               \
    }
    SUPER()
#undef GO
...
void ResetSpecialCaseMainElf(elfheader_t* h)
{
    ...
    for (size_t i = 0; i < h->numDynSym; ++i) {
        if (h->DynSym[i].st_info == 17) {
            ...
            } else if (strcmp(symname, "_IO_2_1_stdin_") == 0 && ((void*)sym->st_value + h->delta)) {
            ...
            } else if (strcmp(symname, "_IO_2_1_stdout_") == 0 && ((void*)sym->st_value + h->delta)) {
            ...
            } else if (strcmp(symname, "_IO_stderr_") == 0 && ((void*)sym->st_value + h->delta)) {
                memcpy((void*)sym->st_value + h->delta, stderr, sym->st_size);
                my__IO_2_1_stderr_ = (void*)sym->st_value + h->delta;
                printf_log(LOG_DEBUG, "BOX64: Set @_IO_stderr_ to %p\n", my__IO_2_1_stderr_);
            } else if (strcmp(symname, "_IO_stdin_") == 0 && ((void*)sym->st_value + h->delta)) {
                memcpy((void*)sym->st_value + h->delta, stdin, sym->st_size);
                my__IO_2_1_stdin_ = (void*)sym->st_value + h->delta;
                printf_log(LOG_DEBUG, "BOX64: Set @_IO_stdin_ to %p\n", my__IO_2_1_stdin_);
            } else if (strcmp(symname, "_IO_stdout_") == 0 && ((void*)sym->st_value + h->delta)) {
                memcpy((void*)sym->st_value + h->delta, stdout, sym->st_size);
                my__IO_2_1_stdout_ = (void*)sym->st_value + h->delta;
