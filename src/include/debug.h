...
#define printf_log(L, ...)                                    \
    do {                                                      \
        if ((L) <= box64_log) { printf_ftrace(__VA_ARGS__); } \
    } while (0)

#define printf_dump(L, ...)                                                   \
    do {                                                                      \
        if (box64_dump || ((L) <= box64_log)) { printf_ftrace(__VA_ARGS__); } \
    } while (0)

#define printf_dlsym(L, ...)                                                   \
    do {                                                                       \
        if (dlsym_error || ((L) <= box64_log)) { printf_ftrace(__VA_ARGS__); } \
    } while (0)

#define dynarec_log(L, ...)                                           \
    do {                                                              \
        if ((L) <= box64_dynarec_log) { printf_ftrace(__VA_ARGS__); } \
    } while (0)
