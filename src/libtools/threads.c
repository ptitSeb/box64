...
#else
#define PTHREAD_ATTR_ALIGN(A)            \
    pthread_attr_t aligned_attr = { 0 }; \
    if (A) memcpy(&aligned_attr, A, 56)
#define PTHREAD_ATTR_UNALIGN(A) \
    if (A) memcpy(A, &aligned_attr, 56)
...
#define GO(A) \
    if (my_cleanup_routine_fct_##A == (uintptr_t)fct) return my_cleanup_routine_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_cleanup_routine_fct_##A == 0) {           \
        my_cleanup_routine_fct_##A = (uintptr_t)fct; \
        return my_cleanup_routine_##A;               \
    }
    SUPER()
#undef GO
...
static void* findkey_dtorFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_key_dtor_fct_##A == (uintptr_t)fct) return my_key_dtor_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_key_dtor_fct_##A == 0) {           \
        my_key_dtor_fct_##A = (uintptr_t)fct; \
        return my_key_dtor_##A;               \
    }
    SUPER()
#undef GO
