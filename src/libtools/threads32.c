...
static pthread_cond_t* get_cond(void* cond)
{
    ...
    if(k==kh_end(mapcond)) {
        printf_log(LOG_DEBUG, "Note: phtread_cond not found, create a new empty one\n");
...
pthread_mutex_t* getAlignedMutex(pthread_mutex_t* m)
{
    ...
    printf_log(LOG_INFO, "Warning, fallback on aligned mutex %p\n", m);
...
EXPORT int my32_pthread_mutex_destroy(pthread_mutex_t *m)
{
    ...
    if(fake->__kind!=KIND_SIGN) {
        printf_log(LOG_NONE, "Warning, fallback on alligned mutex %p\n", m);
        errno = EINVAL;
        return -1;
    }
