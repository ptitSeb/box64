    ...
    if (malloc_hack_2)
        if (getMmapped((uintptr_t)p) || (!p && ALLOC && real_realloc)) {
            ...
            if (real_realloc && ALLOC) {
                ...
                while (s && !(getProtection((uintptr_t)p + s) & PROT_READ)) {
                    if (s > box64_pagesize)
                        s -= box64_pagesize;
                    else
                        s = 0;
...
#define GO(A, B)                             \
    if (!strcmp(symname, #A))                \
        ++hooked;                            \
    else if (!strcmp(symname, "__libc_" #A)) \
        ++hooked;
#define GO2(A, B)
            SUPER()
#undef GO
...
#define GO(A, B)                                                                                           \
    if (!strcmp(symname, "__libc_" #A)) {                                                                  \
        uintptr_t alt = AddCheckBridge(my_context->system, B, A, 0, #A);                                   \
        printf_log(LOG_DEBUG, "Redirecting %s function from %p (%s)\n", symname, (void*)offs, ElfName(h)); \
        addRelocJmp((void*)offs, (void*)alt, sz, "__libc_" #A, h, NULL);                                   \
    }
#define GO2(A, B)
                SUPER()
#undef GO
...
#define GO(A, B)                                                                                           \
    if (!strcmp(symname, #A)) {                                                                            \
        uintptr_t alt = AddCheckBridge(my_context->system, B, A, 0, #A);                                   \
        printf_log(LOG_DEBUG, "Redirecting %s function from %p (%s)\n", symname, (void*)offs, ElfName(h)); \
        addRelocJmp((void*)offs, (void*)alt, sz, #A, h, &real_##A);                                        \
    }
#define GO2(A, B)                                                                                          \
    if (!strcmp(symname, #A) && (box64_malloc_hack > 1)) {                                                 \
        uintptr_t alt = AddCheckBridge(my_context->system, B, my_##A, 0, "my_" #A);                        \
        printf_log(LOG_DEBUG, "Redirecting %s function from %p (%s)\n", symname, (void*)offs, ElfName(h)); \
        addRelocJmp((void*)offs, (void*)alt, sz, "my_" #A, h, &real_##A);                                  \
    }
                SUPER()
#undef GO
#undef GO2
...
#define GO(A, B)
#define GO2(A, B)                          \
    box_##A = (B##_t)dlsym(RTLD_NEXT, #A); \
    if (box_##A == (B##_t)A) box_##A = NULL;
    SUPER()
#undef GO2
#undef GO
#endif
