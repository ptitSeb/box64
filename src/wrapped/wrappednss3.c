...
static void* find_CERT_StringFromCertFcn_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_CERT_StringFromCertFcn_fct_##A == (uintptr_t)fct) return my_CERT_StringFromCertFcn_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_CERT_StringFromCertFcn_fct_##A == 0) {           \
        my_CERT_StringFromCertFcn_fct_##A = (uintptr_t)fct; \
        return my_CERT_StringFromCertFcn_##A;               \
    }
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_CERT_StringFromCertFcn_##A == fct) return (void*)my_CERT_StringFromCertFcn_fct_##A;
    SUPER()
#undef GO
...
static void* find_CERTChainVerifyCallbackFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_CERTChainVerifyCallbackFunc_fct_##A == (uintptr_t)fct) return my_CERTChainVerifyCallbackFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                    \
    if (my_CERTChainVerifyCallbackFunc_fct_##A == 0) {           \
        my_CERTChainVerifyCallbackFunc_fct_##A = (uintptr_t)fct; \
        return my_CERTChainVerifyCallbackFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* reverse_CERTChainVerifyCallbackFunc_Fct(library_t* lib, void* fct)
{
    ...
    if (CheckBridged(lib->w.bridge, fct))
        return (void*)CheckBridged(lib->w.bridge, fct);
#define GO(A) \
    if (my_CERTChainVerifyCallbackFunc_##A == fct) return (void*)my_CERTChainVerifyCallbackFunc_fct_##A;
    SUPER()
#undef GO
...
static void* find_PORTCharConversionWSwapFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_PORTCharConversionWSwapFunc_fct_##A == (uintptr_t)fct) return my_PORTCharConversionWSwapFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                    \
    if (my_PORTCharConversionWSwapFunc_fct_##A == 0) {           \
        my_PORTCharConversionWSwapFunc_fct_##A = (uintptr_t)fct; \
        return my_PORTCharConversionWSwapFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_NSS_ShutdownFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_NSS_ShutdownFunc_fct_##A == (uintptr_t)fct) return my_NSS_ShutdownFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_NSS_ShutdownFunc_fct_##A == 0) {           \
        my_NSS_ShutdownFunc_fct_##A = (uintptr_t)fct; \
        return my_NSS_ShutdownFunc_##A;               \
    }
    SUPER()
#undef GO
