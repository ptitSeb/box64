...
#define GO(A) \
    if (my_SECKEYGetPasswordKey_fct_##A == (uintptr_t)fct) return my_SECKEYGetPasswordKey_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_SECKEYGetPasswordKey_fct_##A == 0) {           \
        my_SECKEYGetPasswordKey_fct_##A = (uintptr_t)fct; \
        return my_SECKEYGetPasswordKey_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_digestOpenFn_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_digestOpenFn_fct_##A == (uintptr_t)fct) return my_digestOpenFn_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_digestOpenFn_fct_##A == 0) {           \
        my_digestOpenFn_fct_##A = (uintptr_t)fct; \
        return my_digestOpenFn_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_digestCloseFn_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_digestCloseFn_fct_##A == (uintptr_t)fct) return my_digestCloseFn_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_digestCloseFn_fct_##A == 0) {           \
        my_digestCloseFn_fct_##A = (uintptr_t)fct; \
        return my_digestCloseFn_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_digestIOFn_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_digestIOFn_fct_##A == (uintptr_t)fct) return my_digestIOFn_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_digestIOFn_fct_##A == 0) {           \
        my_digestIOFn_fct_##A = (uintptr_t)fct; \
        return my_digestIOFn_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_SEC_PKCS12NicknameCollisionCallback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_SEC_PKCS12NicknameCollisionCallback_fct_##A == (uintptr_t)fct) return my_SEC_PKCS12NicknameCollisionCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                                            \
    if (my_SEC_PKCS12NicknameCollisionCallback_fct_##A == 0) {           \
        my_SEC_PKCS12NicknameCollisionCallback_fct_##A = (uintptr_t)fct; \
        return my_SEC_PKCS12NicknameCollisionCallback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_SEC_PKCS12EncoderOutputCallback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_SEC_PKCS12EncoderOutputCallback_fct_##A == (uintptr_t)fct) return my_SEC_PKCS12EncoderOutputCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                                        \
    if (my_SEC_PKCS12EncoderOutputCallback_fct_##A == 0) {           \
        my_SEC_PKCS12EncoderOutputCallback_fct_##A = (uintptr_t)fct; \
        return my_SEC_PKCS12EncoderOutputCallback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_NSSCMSContentCallback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_NSSCMSContentCallback_fct_##A == (uintptr_t)fct) return my_NSSCMSContentCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                              \
    if (my_NSSCMSContentCallback_fct_##A == 0) {           \
        my_NSSCMSContentCallback_fct_##A = (uintptr_t)fct; \
        return my_NSSCMSContentCallback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_PK11PasswordFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_PK11PasswordFunc_fct_##A == (uintptr_t)fct) return my_PK11PasswordFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_PK11PasswordFunc_fct_##A == 0) {           \
        my_PK11PasswordFunc_fct_##A = (uintptr_t)fct; \
        return my_PK11PasswordFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_NSSCMSGetDecryptKeyCallback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_NSSCMSGetDecryptKeyCallback_fct_##A == (uintptr_t)fct) return my_NSSCMSGetDecryptKeyCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                                    \
    if (my_NSSCMSGetDecryptKeyCallback_fct_##A == 0) {           \
        my_NSSCMSGetDecryptKeyCallback_fct_##A = (uintptr_t)fct; \
        return my_NSSCMSGetDecryptKeyCallback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_CERTImportCertificateFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_CERTImportCertificateFunc_fct_##A == (uintptr_t)fct) return my_CERTImportCertificateFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_CERTImportCertificateFunc_fct_##A == 0) {           \
        my_CERTImportCertificateFunc_fct_##A = (uintptr_t)fct; \
        return my_CERTImportCertificateFunc_##A;               \
    }
    SUPER()
#undef GO
