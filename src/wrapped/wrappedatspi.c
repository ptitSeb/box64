...
static void* find_AtspiEventListenerCB_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_AtspiEventListenerCB_fct_##A == (uintptr_t)fct) return my_AtspiEventListenerCB_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_AtspiEventListenerCB_fct_##A == 0) {           \
        my_AtspiEventListenerCB_fct_##A = (uintptr_t)fct; \
        return my_AtspiEventListenerCB_##A;               \
    }
    SUPER()
#undef GO
