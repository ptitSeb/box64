...
#define GO(A) \
    if (my_destroy_user_data_fct_##A == (uintptr_t)fct) return my_destroy_user_data_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_destroy_user_data_fct_##A == 0) {           \
        my_destroy_user_data_fct_##A = (uintptr_t)fct; \
        return my_destroy_user_data_##A;               \
    }
    SUPER()
#undef GO
