...
static void* findGClosureNotifyFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_GClosureNotify_fct_##A == (uintptr_t)fct) return my_GClosureNotify_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_GClosureNotify_fct_##A == 0) {           \
        my_GClosureNotify_fct_##A = (uintptr_t)fct; \
        return my_GClosureNotify_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusGProxyCallNotifyFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusGProxyCallNotify_fct_##A == (uintptr_t)fct) return my_DBusGProxyCallNotify_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_DBusGProxyCallNotify_fct_##A == 0) {           \
        my_DBusGProxyCallNotify_fct_##A = (uintptr_t)fct; \
        return my_DBusGProxyCallNotify_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusGTypeSpecializedCollectionIteratorFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusGTypeSpecializedCollectionIterator_fct_##A == (uintptr_t)fct) return my_DBusGTypeSpecializedCollectionIterator_##A;
    SUPER()
#undef GO
#define GO(A)                                                               \
    if (my_DBusGTypeSpecializedCollectionIterator_fct_##A == 0) {           \
        my_DBusGTypeSpecializedCollectionIterator_fct_##A = (uintptr_t)fct; \
        return my_DBusGTypeSpecializedCollectionIterator_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusGTypeSpecializedMapIteratorFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusGTypeSpecializedMapIterator_fct_##A == (uintptr_t)fct) return my_DBusGTypeSpecializedMapIterator_##A;
    SUPER()
#undef GO
#define GO(A)                                                        \
    if (my_DBusGTypeSpecializedMapIterator_fct_##A == 0) {           \
        my_DBusGTypeSpecializedMapIterator_fct_##A = (uintptr_t)fct; \
        return my_DBusGTypeSpecializedMapIterator_##A;               \
    }
    SUPER()
#undef GO
    printf_log(LOG_NONE, "Warning, no more slot for dbus-glib1 DBusGTypeSpecializedCollectionIterator callback\n");
