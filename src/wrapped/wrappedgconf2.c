...
#define GO(A) \
    if (my_GFreeFct_fct_##A == (uintptr_t)fct) return my_GFreeFct_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_GFreeFct_fct_##A == 0) {           \
        my_GFreeFct_fct_##A = (uintptr_t)fct; \
        return my_GFreeFct_##A;               \
    }
    SUPER()
#undef GO
...
static void* findGConfClientNotifyFuncFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_GConfClientNotifyFunc_fct_##A == (uintptr_t)fct) return my_GConfClientNotifyFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                              \
    if (my_GConfClientNotifyFunc_fct_##A == 0) {           \
        my_GConfClientNotifyFunc_fct_##A = (uintptr_t)fct; \
        return my_GConfClientNotifyFunc_##A;               \
    }
    SUPER()
#undef GO
