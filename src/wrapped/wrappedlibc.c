...
{
    (void)emu;
    FTS* fts = fts_open(path, options, findcompareFct(c));
    if (!fts) return NULL;
    unalign_all_ftsent(fts);
    return fts;
}
EXPORT void* my_fts64_open(x64emu_t* emu, void* path, int options, void* c) __attribute__((alias("my_fts_open")));

EXPORT void* my_fts_children(x64emu_t* emu, void* fts, int instr)
{
    (void)emu;
    align_all_ftsent(fts);
    void* ent = fts_children(fts, instr);
    unalign_all_ftsent(fts);
    return ent;
}
...
EXPORT int my_fts_set(x64emu_t* emu, void* fts, void* ftsent, int instr)
{
    (void)emu;
    AlignFTSENT(ftsent);
    int r = fts_set(fts, ftsent, instr);
    UnalignFTSENT(ftsent);
    return r;
}
...
EXPORT void* my_mmap64(x64emu_t* emu, void *addr, size_t length, int prot, int flags, int fd, ssize_t offset)
{
    ...
    if(ret!=MAP_FAILED) {
        ...
        if(emu) {
            ...
            setGuestProtection((uintptr_t)ret, length, prot, 1);
        } else
    ...
    if(ret!=(void*)-1) {
        ...
        if(ret==old_addr) {
            if(old_size && old_size<new_size) {
                ...
                setGuestProtection((uintptr_t)ret+old_size, new_size-old_size, prot, 1);
                #ifdef DYNAREC
                if(BOX64ENV(dynarec))
                ...
                freeGuestProtection((uintptr_t)ret+new_size, old_size-new_size);
                #ifdef DYNAREC
                if(BOX64ENV(dynarec))
                ...
                setGuestProtection((uintptr_t)ret, new_size, prot, 1);
                #ifdef DYNAREC
                if(BOX64ENV(dynarec))
            ...
            && ((flags&MREMAP_DONTUNMAP)==0)
                ...
                freeGuestProtection((uintptr_t)old_addr, old_size);
                #ifdef DYNAREC
                if(BOX64ENV(dynarec))
...
EXPORT int my_munmap(x64emu_t* emu, void* addr, size_t length)
{
    ...
    if(!ret) {
        ...
        if(partial_host_pages) {
            setGuestProtection(start, length, 0, 0);
            if(unmap_length)
                freeGuestProtection(unmap_start, unmap_length);
        } else
            freeGuestProtection(start, length);
...
EXPORT int my_mprotect(x64emu_t* emu, void *addr, unsigned long len, int prot)
{
    ...
    if(box64_pagesize == X86_PAGE_SIZE) {
        ...
        if(!ret && len) {
            ...
            setGuestProtection(start, len, prot, 0);
        }
        return ret;
    }
    ...
    if(host_end - host_start == box64_pagesize && (start != host_start || end != host_end)) {
        ...
        if(!ret) {
            ...
            setGuestProtection(start, end - start, prot, 0);
        }
        return ret;
    }
