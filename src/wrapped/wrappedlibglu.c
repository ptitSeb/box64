...
#define GO(A) \
    if (my_glu_callback_fct_##A == (uintptr_t)fct) return my_glu_callback_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_glu_callback_fct_##A == 0) {           \
        my_glu_callback_fct_##A = (uintptr_t)fct; \
        return my_glu_callback_##A;               \
    }
    SUPER()
#undef GO
...
static void* findglu_callback4Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glu_callback4_fct_##A == (uintptr_t)fct) return my_glu_callback4_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glu_callback4_fct_##A == 0) {           \
        my_glu_callback4_fct_##A = (uintptr_t)fct; \
        return my_glu_callback4_##A;               \
    }
    SUPER()
#undef GO
...
static void* findglu_callback5Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glu_callback5_fct_##A == (uintptr_t)fct) return my_glu_callback5_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glu_callback5_fct_##A == 0) {           \
        my_glu_callback5_fct_##A = (uintptr_t)fct; \
        return my_glu_callback5_##A;               \
    }
    SUPER()
#undef GO
