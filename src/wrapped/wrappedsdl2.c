...
static void* find_eventfilter_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_eventfilter_fct_##A == (uintptr_t)fct) return my_eventfilter_##A;
    SUPER()
#undef GO
#define GO(A)                                    \
    if (my_eventfilter_fct_##A == 0) {           \
        my_eventfilter_fct_##A = (uintptr_t)fct; \
        return my_eventfilter_##A;               \
    }
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_eventfilter_##A == fct) return (void*)my_eventfilter_fct_##A;
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_LogOutput_fct_##A == (uintptr_t)fct) return my_LogOutput_##A;
    SUPER()
#undef GO
#define GO(A)                                  \
    if (my_LogOutput_fct_##A == 0) {           \
        my_LogOutput_fct_##A = (uintptr_t)fct; \
        return my_LogOutput_##A;               \
    }
    SUPER()
#undef GO
...
static void* reverse_LogOutput_Fct(void* fct)
{
    ...
    if (CheckBridged(my_lib->w.bridge, fct))
        return (void*)CheckBridged(my_lib->w.bridge, fct);
#define GO(A) \
    if (my_LogOutput_##A == fct) return (void*)my_LogOutput_fct_##A;
    SUPER()
#undef GO
...
static const sdl2_tls_dtor dtor_cb[nb_once] = {
    tls_dtor_callback_0, tls_dtor_callback_1, tls_dtor_callback_2, tls_dtor_callback_3, tls_dtor_callback_4, tls_dtor_callback_5, tls_dtor_callback_6, tls_dtor_callback_7, tls_dtor_callback_8, tls_dtor_callback_9, tls_dtor_callback_10, tls_dtor_callback_11, tls_dtor_callback_12, tls_dtor_callback_13, tls_dtor_callback_14, tls_dtor_callback_15
};
