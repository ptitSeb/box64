...
static void updateInstance(x64emu_t* emu, vulkan_my_t* my)
{
    void* p;
#define GO(A, W)                                            \
    p = my_context->vkprocaddress(my->currentInstance, #A); \
    if (p) my->A = p;
    SUPER()
#undef GO
    symbol1_t* s;
...
#define GO(A, W) \
    if (!strcmp(rname, #A)) my->A = (W)my->vkGetDeviceProcAddr(device, name);
        SUPER()
#undef GO
    }
    if (!symbol) {
...
// need to update symbol link maybe
#define GO(A, W)                                                                   \
    if (!strcmp(rname, #A)) my->A = (W)my_context->vkprocaddress(instance, rname); \
    ;
        SUPER()
#undef GO
    }
...
// need to update symbol link maybe
#define GO(A, W) \
    if (!strcmp(rname, #A)) my->A = (W)getaddr(rname);
        SUPER()
#undef GO
    }
    return resolveSymbol(emu, symbol, rname);
...
#define GO(A) \
    if (my_Allocation_fct_##A == (uintptr_t)fct) return my_Allocation_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_Allocation_fct_##A == 0) {           \
        my_Allocation_fct_##A = (uintptr_t)fct; \
        return my_Allocation_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_Reallocation_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_Reallocation_fct_##A == (uintptr_t)fct) return my_Reallocation_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_Reallocation_fct_##A == 0) {           \
        my_Reallocation_fct_##A = (uintptr_t)fct; \
        return my_Reallocation_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_InternalAllocNotification_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_InternalAllocNotification_fct_##A == (uintptr_t)fct) return my_InternalAllocNotification_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_InternalAllocNotification_fct_##A == 0) {           \
        my_InternalAllocNotification_fct_##A = (uintptr_t)fct; \
        return my_InternalAllocNotification_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_InternalFreeNotification_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_InternalFreeNotification_fct_##A == (uintptr_t)fct) return my_InternalFreeNotification_##A;
    SUPER()
#undef GO
#define GO(A)                                                 \
    if (my_InternalFreeNotification_fct_##A == 0) {           \
        my_InternalFreeNotification_fct_##A = (uintptr_t)fct; \
        return my_InternalFreeNotification_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DebugReportCallbackEXT_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DebugReportCallbackEXT_fct_##A == (uintptr_t)fct) return my_DebugReportCallbackEXT_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_DebugReportCallbackEXT_fct_##A == 0) {           \
        my_DebugReportCallbackEXT_fct_##A = (uintptr_t)fct; \
        return my_DebugReportCallbackEXT_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DebugUtilsMessengerCallback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DebugUtilsMessengerCallback_fct_##A == (uintptr_t)fct) return my_DebugUtilsMessengerCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                                    \
    if (my_DebugUtilsMessengerCallback_fct_##A == 0) {           \
        my_DebugUtilsMessengerCallback_fct_##A = (uintptr_t)fct; \
        return my_DebugUtilsMessengerCallback_##A;               \
    }
    SUPER()
#undef GO
