...
GOM(g_action_map_add_action_entries, vFEppip)
...
//GOM(g_application_command_line_print, vFEppV)
//GOM(g_application_command_line_printerr, vFEppV)
...
//GOM(g_buffered_input_stream_fill_async, vFEplippp)
...
GO(g_bus_own_name_on_connection_with_closures, uFppupp)   //should closure be wrapped?
GO(g_bus_own_name_with_closures, uFipuppp)    //should closure be wrapped?
...
GO(g_bus_watch_name_on_connection_with_closures, uFppupp)   //should closure be wrapped?
GO(g_bus_watch_name_with_closures, uFipupp) //should closure be wrapped?
...
//GOM(g_data_input_stream_read_line_async, vFEpippp)
...
//GOM(g_data_input_stream_read_until_async, vFEppippp)
GO(g_data_input_stream_read_until_finish, pFpppp)
GO(g_data_input_stream_read_upto, pFpplppp)
//GOM(g_data_input_stream_read_upto_async, vFEpplippp)
...
//GOM(g_dbus_address_get_stream, vFEpppp)
...
GOM(g_dbus_connection_call_with_unix_fd_list, vFEpppppppuipppp)
...
GOM(g_dbus_connection_send_message_with_reply, vFEppuipppp) // 5th pointer point to volatile uint32
...
//GOM(g_dbus_error_set_dbus_error, vFEppppV)
//GOM(g_dbus_error_set_dbus_error_valist, vFEppppA)
...
//GOM(g_dbus_interface_skeleton_get_vtable, pFEp)
...
//GOM(g_dbus_message_new_method_error, pFEpppV)
GO(g_dbus_message_new_method_error_literal, pFppp)
//GOM(g_dbus_message_new_method_error_valist, pFEpppA)
...
//GOM(g_dbus_object_manager_client_new, vFEpupppppppp)
GO(g_dbus_object_manager_client_new_finish, pFpp)
//GOM(g_dbus_object_manager_client_new_for_bus, vFEiupppppppp)
GO(g_dbus_object_manager_client_new_for_bus_finish, pFpp)
GOM(g_dbus_object_manager_client_new_for_bus_sync, pFEiuppppppp)
//GOM(g_dbus_object_manager_client_new_sync, pFEpuppppppp)
//GOM(g_dbus_object_manager_get_interface, pFEppp)
...
//GOM(g_desktop_app_info_launch_uris_as_manager, iFEpppuppppp)
...
//GOM(g_drive_eject, vFEpuppp)
GO(g_drive_eject_finish, iFppp)
//GOM(g_drive_eject_with_operation, vFEpupppp)
...
//GOM(g_file_enumerator_next_files_async, vFEpiippp)
...
//GOM(g_file_input_stream_query_info_async, vFEppippp)
...
//GOM(g_file_load_partial_contents_async, vFEppppp)
...
//GOM(g_file_output_stream_query_info_async, vFEppippp)
...
//GOM(g_file_query_default_handler_async, vFEpippp)
...
//GOM(g_file_stop_mountable, vFEpupppp)
...
//GOM(g_input_stream_close_async, vFEpippp)
...
//GOM(g_input_stream_read_bytes_async, vFEpLippp)
...
//GOM(g_input_stream_skip_async, vFEpLippp)
...
//GOM(g_io_scheduler_job_send_to_mainloop, iFEpppp)
//GOM(g_io_scheduler_job_send_to_mainloop_async, vFEpppp)
//GOM(g_io_scheduler_push_job, vFEpppip)
GO(g_io_stream_clear_pending, vFp)
GO(g_io_stream_close, iFppp)
//GOM(g_io_stream_close_async, vFEpippp)
...
//GOM(g_io_stream_splice_async, vFEppuippp)
GO(g_io_stream_splice_finish, iFpp)
GO(g_io_stream_splice_flags_get_type, LFv)
GO(g_keyfile_settings_backend_new, pFppp)
...
//GOM(g_loadable_icon_load_async, vFEpippp)
...
//GOM(g_memory_output_stream_new, pFEpLpp)
...
GO(g_memory_settings_backend_new, pFv)
...
//GOM(g_menu_item_get_attribute, iFEpppV)
...
//GOM(g_menu_item_set_action_and_target, vFEpppV)
GO(g_menu_item_set_action_and_target_value, vFppp)
//GOM(g_menu_item_set_attribute, vFEpppV)
...
//GOM(g_menu_model_get_item_attribute, iFEpippV)
...
//GOM(g_mount_eject, vFEpuppp)
GO(g_mount_eject_finish, iFppp)
//GOM(g_mount_eject_with_operation, vFEpupppp)
...
//GOM(g_mount_guess_content_type, vFEpippp)
...
//GOM(g_network_monitor_can_reach_async, vFEppppp)
GO(g_network_monitor_can_reach_finish, iFppp)
GO(g_network_monitor_get_connectivity, uFp)
...
//GOM(g_notification_add_button_with_target, vFEppppV)
...
//GOM(g_notification_set_default_action_and_target, vFEpppV)
...
//GOM(g_permission_acquire_async, vFEpppp)
...
//GOM(g_permission_release_async, vFEpppp)
GO(g_permission_release_finish, iFppp)
GO(g_pollable_input_stream_can_poll, iFp)
//GOM(g_pollable_input_stream_create_source, pFEpp)
...
//GOM(g_pollable_output_stream_create_source, pFEpp)
...
//GOM(g_pollable_source_new, pFEp)
//GOM(g_pollable_source_new_full, pFEppp)
...
//GOM(g_proxy_connect_async, vFEpppppp)
...
//GOM(g_proxy_resolver_lookup_async, vFEppppp)
...
GO(g_settings_backend_changed, vFppp)
GO(g_settings_backend_changed_tree, vFppp)
GO(g_settings_backend_flatten_tree, vFpppp)
GO(g_settings_backend_get_default, pFv)
GO(g_settings_backend_get_type, LFv)
GO(g_settings_backend_keys_changed, vFpppp)
GO(g_settings_backend_path_changed, vFppp)
GO(g_settings_backend_path_writable_changed, vFpp)
GO(g_settings_backend_writable_changed, vFpp)
GO(g_settings_bind, vFppppu)
GO(g_settings_bind_flags_get_type, LFv)
//GOM(g_settings_bind_with_mapping, vFEppppupppp)
GO(g_settings_bind_writable, vFppppi)
GO(g_settings_create_action, pFpp)
GO(g_settings_delay, vFp)
//GOM(g_settings_get, vFEpppV)
...
//GOM(g_settings_get_mapped, pFEpppp)
...
//GOM(g_simple_action_group_add_entries, vFEppip)
...
//GOM(g_socket_address_enumerator_next_async, vFEpppp)
...
//GOM(g_socket_connection_connect_async, vFEppppp)
...
//GOM(g_socket_create_source, pFEpup)
...
//GOM(g_socket_listener_accept_async, vFEpppp)
GO(g_socket_listener_accept_finish, pFpppp)
GO(g_socket_listener_accept_socket, pFpppp)
//GOM(g_socket_listener_accept_socket_async, vFEpppp)
...
//GOM(g_subprocess_communicate_async, vFEppppp)
GO(g_subprocess_communicate_finish, iFppppp)
GO(g_subprocess_communicate_utf8, iFpppppp)
//GOM(g_subprocess_communicate_utf8_async, vFEppppp)
...
//GOM(g_subprocess_launcher_set_child_setup, vFEpppp)
...
//GOM(g_subprocess_launcher_spawn, pFEpppV)
GOM(g_subprocess_launcher_spawnv, pFEppp)
...
//GOM(g_subprocess_new, pFEuppV)
GOM(g_subprocess_newv, pFEpup)
GO(g_subprocess_send_signal, vFpi)
GO(g_subprocess_wait, iFppp)
//GOM(g_subprocess_wait_async, vFEpppp)
GO(g_subprocess_wait_check, iFppp)
//GOM(g_subprocess_wait_check_async, vFEpppp)
GO(g_subprocess_wait_check_finish, iFppp)
GO(g_subprocess_wait_finish, iFppp)
//GOM(g_task_attach_source, vFEppp)
...
//GOM(g_task_report_error, vFEppppp)
//GOM(g_task_report_new_error, vFEppppuipV)
...
//GOM(g_tls_connection_handshake_async, vFEpippp)
...
//GOM(g_tls_database_verify_chain_async, vFEpppppuppp)
...
//GOM(g_tls_interaction_ask_password_async, vFEppppp)
...
//GOM(g_tls_interaction_request_certificate_async, vFEppuppp)
...
//GOM(g_tls_password_set_value_full, vFEpplp)
...
//GOM(g_unix_connection_receive_credentials_async, vFEpppp)
...
//GOM(g_unix_connection_send_credentials_async, vFEpppp)
...
//GOM(g_volume_eject, vFEpuppp)
GO(g_volume_eject_finish, iFppp)
//GOM(g_volume_eject_with_operation, vFEpupppp)
