...
#define GO(A) \
    if (my_dispcallback_fct_##A == (uintptr_t)fct) return my_dispcallback_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_dispcallback_fct_##A == 0) {           \
        my_dispcallback_fct_##A = (uintptr_t)fct; \
        return my_dispcallback_##A;               \
    }
    SUPER()
#undef GO
