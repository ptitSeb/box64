...
#define GO(A) \
    if (my_buffer_message_fct_##A == (uintptr_t)fct) return my_buffer_message_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_buffer_message_fct_##A == 0) {           \
        my_buffer_message_fct_##A = (uintptr_t)fct; \
        return my_buffer_message_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_draw_close_path_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_draw_close_path_fct_##A == (uintptr_t)fct) return my_draw_close_path_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_draw_close_path_fct_##A == 0) {           \
        my_draw_close_path_fct_##A = (uintptr_t)fct; \
        return my_draw_close_path_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_draw_cubic_to_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_draw_cubic_to_fct_##A == (uintptr_t)fct) return my_draw_cubic_to_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_draw_cubic_to_fct_##A == 0) {           \
        my_draw_cubic_to_fct_##A = (uintptr_t)fct; \
        return my_draw_cubic_to_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_draw_line_or_move_to_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_draw_line_or_move_to_fct_##A == (uintptr_t)fct) return my_draw_line_or_move_to_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_draw_line_or_move_to_fct_##A == 0) {           \
        my_draw_line_or_move_to_fct_##A = (uintptr_t)fct; \
        return my_draw_line_or_move_to_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_draw_quadratic_to_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_draw_quadratic_to_fct_##A == (uintptr_t)fct) return my_draw_quadratic_to_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_draw_quadratic_to_fct_##A == 0) {           \
        my_draw_quadratic_to_fct_##A = (uintptr_t)fct; \
        return my_draw_quadratic_to_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_reference_table_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_reference_table_fct_##A == (uintptr_t)fct) return my_reference_table_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_reference_table_fct_##A == 0) {           \
        my_reference_table_fct_##A = (uintptr_t)fct; \
        return my_reference_table_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_combining_class_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_combining_class_fct_##A == (uintptr_t)fct) return my_unicode_combining_class_##A;
    SUPER()
#undef GO
#define GO(A)                                                \
    if (my_unicode_combining_class_fct_##A == 0) {           \
        my_unicode_combining_class_fct_##A = (uintptr_t)fct; \
        return my_unicode_combining_class_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_compose_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_compose_fct_##A == (uintptr_t)fct) return my_unicode_compose_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_unicode_compose_fct_##A == 0) {           \
        my_unicode_compose_fct_##A = (uintptr_t)fct; \
        return my_unicode_compose_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_decompose_compatibility_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_decompose_compatibility_fct_##A == (uintptr_t)fct) return my_unicode_decompose_compatibility_##A;
    SUPER()
#undef GO
#define GO(A)                                                        \
    if (my_unicode_decompose_compatibility_fct_##A == 0) {           \
        my_unicode_decompose_compatibility_fct_##A = (uintptr_t)fct; \
        return my_unicode_decompose_compatibility_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_decompose_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_decompose_fct_##A == (uintptr_t)fct) return my_unicode_decompose_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_unicode_decompose_fct_##A == 0) {           \
        my_unicode_decompose_fct_##A = (uintptr_t)fct; \
        return my_unicode_decompose_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_eastasian_width_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_eastasian_width_fct_##A == (uintptr_t)fct) return my_unicode_eastasian_width_##A;
    SUPER()
#undef GO
#define GO(A)                                                \
    if (my_unicode_eastasian_width_fct_##A == 0) {           \
        my_unicode_eastasian_width_fct_##A = (uintptr_t)fct; \
        return my_unicode_eastasian_width_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_general_category_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_general_category_fct_##A == (uintptr_t)fct) return my_unicode_general_category_##A;
    SUPER()
#undef GO
#define GO(A)                                                 \
    if (my_unicode_general_category_fct_##A == 0) {           \
        my_unicode_general_category_fct_##A = (uintptr_t)fct; \
        return my_unicode_general_category_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_mirroring_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_mirroring_fct_##A == (uintptr_t)fct) return my_unicode_mirroring_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_unicode_mirroring_fct_##A == 0) {           \
        my_unicode_mirroring_fct_##A = (uintptr_t)fct; \
        return my_unicode_mirroring_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unicode_script_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unicode_script_fct_##A == (uintptr_t)fct) return my_unicode_script_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_unicode_script_fct_##A == 0) {           \
        my_unicode_script_fct_##A = (uintptr_t)fct; \
        return my_unicode_script_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_font_extents_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_font_extents_fct_##A == (uintptr_t)fct) return my_font_extents_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_font_extents_fct_##A == 0) {           \
        my_font_extents_fct_##A = (uintptr_t)fct; \
        return my_font_extents_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_advance_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_advance_fct_##A == (uintptr_t)fct) return my_glyph_advance_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glyph_advance_fct_##A == 0) {           \
        my_glyph_advance_fct_##A = (uintptr_t)fct; \
        return my_glyph_advance_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_advances_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_advances_fct_##A == (uintptr_t)fct) return my_glyph_advances_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_glyph_advances_fct_##A == 0) {           \
        my_glyph_advances_fct_##A = (uintptr_t)fct; \
        return my_glyph_advances_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_kerning_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_kerning_fct_##A == (uintptr_t)fct) return my_glyph_kerning_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glyph_kerning_fct_##A == 0) {           \
        my_glyph_kerning_fct_##A = (uintptr_t)fct; \
        return my_glyph_kerning_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_origin_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_origin_fct_##A == (uintptr_t)fct) return my_glyph_origin_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_glyph_origin_fct_##A == 0) {           \
        my_glyph_origin_fct_##A = (uintptr_t)fct; \
        return my_glyph_origin_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_contour_point_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_contour_pointfct_##A == (uintptr_t)fct) return my_glyph_contour_point##A;
    SUPER()
#undef GO
#define GO(A)                                           \
    if (my_glyph_contour_pointfct_##A == 0) {           \
        my_glyph_contour_pointfct_##A = (uintptr_t)fct; \
        return my_glyph_contour_point##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_extents_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_extents_fct_##A == (uintptr_t)fct) return my_glyph_extents_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glyph_extents_fct_##A == 0) {           \
        my_glyph_extents_fct_##A = (uintptr_t)fct; \
        return my_glyph_extents_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_from_name_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_from_name_fct_##A == (uintptr_t)fct) return my_glyph_from_name_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_glyph_from_name_fct_##A == 0) {           \
        my_glyph_from_name_fct_##A = (uintptr_t)fct; \
        return my_glyph_from_name_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_fct_##A == (uintptr_t)fct) return my_glyph_##A;
    SUPER()
#undef GO
#define GO(A)                              \
    if (my_glyph_fct_##A == 0) {           \
        my_glyph_fct_##A = (uintptr_t)fct; \
        return my_glyph_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glyph_name_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_name_fct_##A == (uintptr_t)fct) return my_glyph_name_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_glyph_name_fct_##A == 0) {           \
        my_glyph_name_fct_##A = (uintptr_t)fct; \
        return my_glyph_name_##A;               \
    }
    SUPER()
#undef GO
    printf_log(LOG_NONE, "Warning, no more slot for libharfbuzz glyph origin callback\n");
...
static void* find_glyph_shape_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_glyph_shape_fct_##A == (uintptr_t)fct) return my_glyph_shape_##A;
    SUPER()
#undef GO
#define GO(A)                                    \
    if (my_glyph_shape_fct_##A == 0) {           \
        my_glyph_shape_fct_##A = (uintptr_t)fct; \
        return my_glyph_shape_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_nominal_glyph_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_nominal_glyph_fct_##A == (uintptr_t)fct) return my_nominal_glyph_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_nominal_glyph_fct_##A == 0) {           \
        my_nominal_glyph_fct_##A = (uintptr_t)fct; \
        return my_nominal_glyph_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_nominal_glyphs_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_nominal_glyphs_fct_##A == (uintptr_t)fct) return my_nominal_glyphs_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_nominal_glyphs_fct_##A == 0) {           \
        my_nominal_glyphs_fct_##A = (uintptr_t)fct; \
        return my_nominal_glyphs_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_variation_glyph_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_variation_glyph_fct_##A == (uintptr_t)fct) return my_variation_glyph_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_variation_glyph_fct_##A == 0) {           \
        my_variation_glyph_fct_##A = (uintptr_t)fct; \
        return my_variation_glyph_##A;               \
    }
    SUPER()
#undef GO
