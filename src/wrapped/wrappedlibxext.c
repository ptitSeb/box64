...
#define GO(A) \
    if (my_exterrorhandle_fct_##A == (uintptr_t)fct) return my_exterrorhandle_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_exterrorhandle_fct_##A == 0) {           \
        my_exterrorhandle_fct_##A = (uintptr_t)fct; \
        return my_exterrorhandle_##A;               \
    }
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_exterrorhandle_##A == fct) return (void*)my_exterrorhandle_fct_##A;
    SUPER()
#undef GO
...
static void* find_create_gc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_create_gc_fct_##A == (uintptr_t)fct) return my_create_gc_##A;
    SUPER()
#undef GO
#define GO(A)                                  \
    if (my_create_gc_fct_##A == 0) {           \
        my_create_gc_fct_##A = (uintptr_t)fct; \
        return my_create_gc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_copy_gc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_copy_gc_fct_##A == (uintptr_t)fct) return my_copy_gc_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_copy_gc_fct_##A == 0) {           \
        my_copy_gc_fct_##A = (uintptr_t)fct; \
        return my_copy_gc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_flush_gc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_flush_gc_fct_##A == (uintptr_t)fct) return my_flush_gc_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_flush_gc_fct_##A == 0) {           \
        my_flush_gc_fct_##A = (uintptr_t)fct; \
        return my_flush_gc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_free_gc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_free_gc_fct_##A == (uintptr_t)fct) return my_free_gc_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_free_gc_fct_##A == 0) {           \
        my_free_gc_fct_##A = (uintptr_t)fct; \
        return my_free_gc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_create_font_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_create_font_fct_##A == (uintptr_t)fct) return my_create_font_##A;
    SUPER()
#undef GO
#define GO(A)                                    \
    if (my_create_font_fct_##A == 0) {           \
        my_create_font_fct_##A = (uintptr_t)fct; \
        return my_create_font_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_free_font_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_free_font_fct_##A == (uintptr_t)fct) return my_free_font_##A;
    SUPER()
#undef GO
#define GO(A)                                  \
    if (my_free_font_fct_##A == 0) {           \
        my_free_font_fct_##A = (uintptr_t)fct; \
        return my_free_font_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_error_string_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_error_string_fct_##A == (uintptr_t)fct) return my_error_string_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_error_string_fct_##A == 0) {           \
        my_error_string_fct_##A = (uintptr_t)fct; \
        return my_error_string_##A;               \
    }
    SUPER()
#undef GO
...
EXPORT void* my_XShmCreateImage(x64emu_t* emu, void* disp, void* vis, uint32_t depth, int32_t fmt, void* data, void* shminfo, uint32_t w, uint32_t h)
{
...
EXPORT int32_t my_XShmPutImage(x64emu_t* emu, void* disp, void* drawable, void* gc, void* image, int32_t src_x, int32_t src_y, int32_t dst_x, int32_t dst_y, uint32_t w, uint32_t h, int32_t sendevt)
{
