...
#define GO(A) \
    if (my_debug_callback_fct_##A == (uintptr_t)fct) return my_debug_callback_##A;
    SUPER()
#undef GO
#define GO(A)                                       \
    if (my_debug_callback_fct_##A == 0) {           \
        my_debug_callback_fct_##A = (uintptr_t)fct; \
        return my_debug_callback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_egl_debug_callback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_egl_debug_callback_fct_##A == (uintptr_t)fct) return my_egl_debug_callback_##A;
    SUPER()
#undef GO
#define GO(A)                                           \
    if (my_egl_debug_callback_fct_##A == 0) {           \
        my_egl_debug_callback_fct_##A = (uintptr_t)fct; \
        return my_egl_debug_callback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_program_callback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_program_callback_fct_##A == (uintptr_t)fct) return my_program_callback_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_program_callback_fct_##A == 0) {           \
        my_program_callback_fct_##A = (uintptr_t)fct; \
        return my_program_callback_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_set_blob_func_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_set_blob_func_fct_##A == (uintptr_t)fct) return my_set_blob_func_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_set_blob_func_fct_##A == 0) {           \
        my_set_blob_func_fct_##A = (uintptr_t)fct; \
        return my_set_blob_func_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_get_blob_func_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_get_blob_func_fct_##A == (uintptr_t)fct) return my_get_blob_func_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_get_blob_func_fct_##A == 0) {           \
        my_get_blob_func_fct_##A = (uintptr_t)fct; \
        return my_get_blob_func_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_glDebugMessageCallback_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glDebugMessageCallback_fct_##A == (vFpp_t)fct) return my_glDebugMessageCallback_##A;
    SUPER()
#undef GO
#define GO(A)                                            \
    if (my_glDebugMessageCallback_fct_##A == 0) {        \
        my_glDebugMessageCallback_fct_##A = (vFpp_t)fct; \
        return my_glDebugMessageCallback_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_glDebugMessageCallbackARB_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glDebugMessageCallbackARB_fct_##A == (vFpp_t)fct) return my_glDebugMessageCallbackARB_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_glDebugMessageCallbackARB_fct_##A == 0) {        \
        my_glDebugMessageCallbackARB_fct_##A = (vFpp_t)fct; \
        return my_glDebugMessageCallbackARB_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_glDebugMessageCallbackAMD_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glDebugMessageCallbackAMD_fct_##A == (vFpp_t)fct) return my_glDebugMessageCallbackAMD_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_glDebugMessageCallbackAMD_fct_##A == 0) {        \
        my_glDebugMessageCallbackAMD_fct_##A = (vFpp_t)fct; \
        return my_glDebugMessageCallbackAMD_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_glDebugMessageCallbackKHR_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glDebugMessageCallbackKHR_fct_##A == (vFpp_t)fct) return my_glDebugMessageCallbackKHR_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_glDebugMessageCallbackKHR_fct_##A == 0) {        \
        my_glDebugMessageCallbackKHR_fct_##A = (vFpp_t)fct; \
        return my_glDebugMessageCallbackKHR_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_eglDebugMessageControlKHR_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_eglDebugMessageControlKHR_fct_##A == (vFpp_t)fct) return my_eglDebugMessageControlKHR_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_eglDebugMessageControlKHR_fct_##A == 0) {        \
        my_eglDebugMessageControlKHR_fct_##A = (vFpp_t)fct; \
        return my_eglDebugMessageControlKHR_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_eglSetBlobCacheFuncsANDROID_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_eglSetBlobCacheFuncsANDROID_fct_##A == (vFppp_t)fct) return my_eglSetBlobCacheFuncsANDROID_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_eglSetBlobCacheFuncsANDROID_fct_##A == 0) {         \
        my_eglSetBlobCacheFuncsANDROID_fct_##A = (vFppp_t)fct; \
        return my_eglSetBlobCacheFuncsANDROID_##A;             \
    }
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_glXSwapIntervalMESA_fct_##A == (iFi_t)fct) return my_glXSwapIntervalMESA_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_glXSwapIntervalMESA_fct_##A == 0) {       \
        my_glXSwapIntervalMESA_fct_##A = (iFi_t)fct; \
        return my_glXSwapIntervalMESA_##A;           \
    }
    SUPER()
#undef GO
...
#define GO(A) \
    if (my_glXSwapIntervalEXT_fct_##A == (vFppi_t)fct) return my_glXSwapIntervalEXT_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_glXSwapIntervalEXT_fct_##A == 0) {         \
        my_glXSwapIntervalEXT_fct_##A = (vFppi_t)fct; \
        return my_glXSwapIntervalEXT_##A;             \
    }
    SUPER()
#undef GO
...
static void* find_glProgramCallbackMESA_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glProgramCallbackMESA_fct_##A == (vFpp_t)fct) return my_glProgramCallbackMESA_##A;
    SUPER()
#undef GO
#define GO(A)                                           \
    if (my_glProgramCallbackMESA_fct_##A == 0) {        \
        my_glProgramCallbackMESA_fct_##A = (vFpp_t)fct; \
        return my_glProgramCallbackMESA_##A;            \
    }
    SUPER()
#undef GO
...
static void* find_glGetVkProcAddrNV_Fct(void* fct)
{
    if (!fct) return fct;
#define GO(A) \
    if (my_glGetVkProcAddrNV_fct_##A == (pFp_t)fct) return my_glGetVkProcAddrNV_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_glGetVkProcAddrNV_fct_##A == 0) {       \
        my_glGetVkProcAddrNV_fct_##A = (pFp_t)fct; \
        return my_glGetVkProcAddrNV_##A;           \
    }
    SUPER()
#undef GO
