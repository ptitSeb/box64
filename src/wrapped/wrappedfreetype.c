...
#define GO(A) \
    if (my_FT_Generic_Finalizer_fct_##A == (uintptr_t)fct) return my_FT_Generic_Finalizer_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_FT_Generic_Finalizer_fct_##A == 0) {           \
        my_FT_Generic_Finalizer_fct_##A = (uintptr_t)fct; \
        return my_FT_Generic_Finalizer_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FTC_Face_Requester_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FTC_Face_Requester_fct_##A == (uintptr_t)fct) return my_FTC_Face_Requester_##A;
    SUPER()
#undef GO
#define GO(A)                                           \
    if (my_FTC_Face_Requester_fct_##A == 0) {           \
        my_FTC_Face_Requester_fct_##A = (uintptr_t)fct; \
        return my_FTC_Face_Requester_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Alloc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Alloc_fct_##A == (uintptr_t)fct) return my_FT_Alloc_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_FT_Alloc_fct_##A == 0) {           \
        my_FT_Alloc_fct_##A = (uintptr_t)fct; \
        return my_FT_Alloc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Free_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Free_fct_##A == (uintptr_t)fct) return my_FT_Free_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_FT_Free_fct_##A == 0) {           \
        my_FT_Free_fct_##A = (uintptr_t)fct; \
        return my_FT_Free_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Realloc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Realloc_fct_##A == (uintptr_t)fct) return my_FT_Realloc_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_FT_Realloc_fct_##A == 0) {           \
        my_FT_Realloc_fct_##A = (uintptr_t)fct; \
        return my_FT_Realloc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Outline_MoveToFunc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Outline_MoveToFunc_fct_##A == (uintptr_t)fct) return my_FT_Outline_MoveToFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                              \
    if (my_FT_Outline_MoveToFunc_fct_##A == 0) {           \
        my_FT_Outline_MoveToFunc_fct_##A = (uintptr_t)fct; \
        return my_FT_Outline_MoveToFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Outline_LineToFunc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Outline_LineToFunc_fct_##A == (uintptr_t)fct) return my_FT_Outline_LineToFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                              \
    if (my_FT_Outline_LineToFunc_fct_##A == 0) {           \
        my_FT_Outline_LineToFunc_fct_##A = (uintptr_t)fct; \
        return my_FT_Outline_LineToFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Outline_ConicToFunc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Outline_ConicToFunc_fct_##A == (uintptr_t)fct) return my_FT_Outline_ConicToFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_FT_Outline_ConicToFunc_fct_##A == 0) {           \
        my_FT_Outline_ConicToFunc_fct_##A = (uintptr_t)fct; \
        return my_FT_Outline_ConicToFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FT_Outline_CubicToFunc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_FT_Outline_CubicToFunc_fct_##A == (uintptr_t)fct) return my_FT_Outline_CubicToFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_FT_Outline_CubicToFunc_fct_##A == 0) {           \
        my_FT_Outline_CubicToFunc_fct_##A = (uintptr_t)fct; \
        return my_FT_Outline_CubicToFunc_##A;               \
    }
    SUPER()
#undef GO
...
static FT_MemoryRec_t* find_FT_MemoryRec_Struct(FT_MemoryRec_t* s)
{
    if (!s) return NULL;
#define GO(A) \
    if (my_FT_MemoryRec_ref_##A == s) return &my_FT_MemoryRec_struct_##A;
    SUPER()
#undef GO
#define GO(A)                                              \
    if (!my_FT_MemoryRec_ref_##A) {                        \
        wrap_FT_MemoryRec(&my_FT_MemoryRec_struct_##A, s); \
        my_FT_MemoryRec_ref_##A = s;                       \
        return &my_FT_MemoryRec_struct_##A;                \
    }
    SUPER()
#undef GO
