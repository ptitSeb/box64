...
#define GO(A) \
    if (my3_ENGINE_ctrl_cb_fct_##A == (uintptr_t)fct) return my3_ENGINE_ctrl_cb_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my3_ENGINE_ctrl_cb_fct_##A == 0) {           \
        my3_ENGINE_ctrl_cb_fct_##A = (uintptr_t)fct; \
        return my3_ENGINE_ctrl_cb_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_cmp_fnc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_cmp_fnc_fct_##A == (uintptr_t)fct) return my3_cmp_fnc_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my3_cmp_fnc_fct_##A == 0) {           \
        my3_cmp_fnc_fct_##A = (uintptr_t)fct; \
        return my3_cmp_fnc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_free_fnc_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_free_fnc_fct_##A == (uintptr_t)fct) return my3_free_fnc_##A;
    SUPER()
#undef GO
#define GO(A)                                  \
    if (my3_free_fnc_fct_##A == 0) {           \
        my3_free_fnc_fct_##A = (uintptr_t)fct; \
        return my3_free_fnc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_id_func_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_id_func_fct_##A == (uintptr_t)fct) return my3_id_func_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
...
static void* find_lock_func_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_lock_func_fct_##A == (uintptr_t)fct) return my3_lock_func_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my3_lock_func_fct_##A == 0) {           \
        my3_lock_func_fct_##A = (uintptr_t)fct; \
        return my3_lock_func_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_passphrase_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_passphrase_fct_##A == (uintptr_t)fct) return my3_passphrase_##A;
    SUPER()
#undef GO
#define GO(A)                                    \
    if (my3_passphrase_fct_##A == 0) {           \
        my3_passphrase_fct_##A = (uintptr_t)fct; \
        return my3_passphrase_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_xnew_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_xnew_fct_##A == (uintptr_t)fct) return my3_xnew_##A;
    SUPER()
#undef GO
#define GO(A)                              \
    if (my3_xnew_fct_##A == 0) {           \
        my3_xnew_fct_##A = (uintptr_t)fct; \
        return my3_xnew_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_d2i_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_d2i_fct_##A == (uintptr_t)fct) return my3_d2i_##A;
    SUPER()
#undef GO
#define GO(A)                             \
    if (my3_d2i_fct_##A == 0) {           \
        my3_d2i_fct_##A = (uintptr_t)fct; \
        return my3_d2i_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_i2d_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_i2d_fct_##A == (uintptr_t)fct) return my3_i2d_##A;
    SUPER()
#undef GO
#define GO(A)                             \
    if (my3_i2d_fct_##A == 0) {           \
        my3_i2d_fct_##A = (uintptr_t)fct; \
        return my3_i2d_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_pem_password_cb_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_pem_password_cb_fct_##A == (uintptr_t)fct) return my3_pem_password_cb_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my3_pem_password_cb_fct_##A == 0) {           \
        my3_pem_password_cb_fct_##A = (uintptr_t)fct; \
        return my3_pem_password_cb_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_verify_cb_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_verify_cb_fct_##A == (uintptr_t)fct) return my3_verify_cb_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my3_verify_cb_fct_##A == 0) {           \
        my3_verify_cb_fct_##A = (uintptr_t)fct; \
        return my3_verify_cb_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_do_all_provided_cb_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my3_do_all_provided_cb_fct_##A == (uintptr_t)fct) return my3_do_all_provided_cb_##A;
    SUPER()
#undef GO
#define GO(A)                                            \
    if (my3_do_all_provided_cb_fct_##A == 0) {           \
        my3_do_all_provided_cb_fct_##A = (uintptr_t)fct; \
        return my3_do_all_provided_cb_##A;               \
    }
    SUPER()
#undef GO
