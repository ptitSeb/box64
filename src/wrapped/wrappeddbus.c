...
#define GO(A) \
    if (my_DBusFreeFunction_fct_##A == (uintptr_t)fct) return my_DBusFreeFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_DBusFreeFunction_fct_##A == 0) {           \
        my_DBusFreeFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusFreeFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DBusHandleMessageFunction_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusHandleMessageFunction_fct_##A == (uintptr_t)fct) return my_DBusHandleMessageFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_DBusHandleMessageFunction_fct_##A == 0) {           \
        my_DBusHandleMessageFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusHandleMessageFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DBusAddTimeoutFunction_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusAddTimeoutFunction_fct_##A == (uintptr_t)fct) return my_DBusAddTimeoutFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_DBusAddTimeoutFunction_fct_##A == 0) {           \
        my_DBusAddTimeoutFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusAddTimeoutFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DBusRemoveTimeoutFunction_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusRemoveTimeoutFunction_fct_##A == (uintptr_t)fct) return my_DBusRemoveTimeoutFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_DBusRemoveTimeoutFunction_fct_##A == 0) {           \
        my_DBusRemoveTimeoutFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusRemoveTimeoutFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DBusTimeoutToggledFunction_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusTimeoutToggledFunction_fct_##A == (uintptr_t)fct) return my_DBusTimeoutToggledFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                   \
    if (my_DBusTimeoutToggledFunction_fct_##A == 0) {           \
        my_DBusTimeoutToggledFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusTimeoutToggledFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_DBusWakeupMainFunction_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusWakeupMainFunction_fct_##A == (uintptr_t)fct) return my_DBusWakeupMainFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                               \
    if (my_DBusWakeupMainFunction_fct_##A == 0) {           \
        my_DBusWakeupMainFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusWakeupMainFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusPendingCallNotifyFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusPendingCallNotifyFunction_fct_##A == (uintptr_t)fct) return my_DBusPendingCallNotifyFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                      \
    if (my_DBusPendingCallNotifyFunction_fct_##A == 0) {           \
        my_DBusPendingCallNotifyFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusPendingCallNotifyFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusDispatchStatusFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusDispatchStatusFunction_fct_##A == (uintptr_t)fct) return my_DBusDispatchStatusFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                   \
    if (my_DBusDispatchStatusFunction_fct_##A == 0) {           \
        my_DBusDispatchStatusFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusDispatchStatusFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusAddWatchFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusAddWatchFunction_fct_##A == (uintptr_t)fct) return my_DBusAddWatchFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                             \
    if (my_DBusAddWatchFunction_fct_##A == 0) {           \
        my_DBusAddWatchFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusAddWatchFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusRemoveWatchFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusRemoveWatchFunction_fct_##A == (uintptr_t)fct) return my_DBusRemoveWatchFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                \
    if (my_DBusRemoveWatchFunction_fct_##A == 0) {           \
        my_DBusRemoveWatchFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusRemoveWatchFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusWatchToggledFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusWatchToggledFunction_fct_##A == (uintptr_t)fct) return my_DBusWatchToggledFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                 \
    if (my_DBusWatchToggledFunction_fct_##A == 0) {           \
        my_DBusWatchToggledFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusWatchToggledFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusObjectPathUnregisterFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusObjectPathUnregisterFunction_fct_##A == (uintptr_t)fct) return my_DBusObjectPathUnregisterFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                         \
    if (my_DBusObjectPathUnregisterFunction_fct_##A == 0) {           \
        my_DBusObjectPathUnregisterFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusObjectPathUnregisterFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusObjectPathMessageFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusObjectPathMessageFunction_fct_##A == (uintptr_t)fct) return my_DBusObjectPathMessageFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                      \
    if (my_DBusObjectPathMessageFunction_fct_##A == 0) {           \
        my_DBusObjectPathMessageFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusObjectPathMessageFunction_##A;               \
    }
    SUPER()
#undef GO
...
static void* finddbus_internal_padFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_dbus_internal_pad_fct_##A == (uintptr_t)fct) return my_dbus_internal_pad_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_dbus_internal_pad_fct_##A == 0) {           \
        my_dbus_internal_pad_fct_##A = (uintptr_t)fct; \
        return my_dbus_internal_pad_##A;               \
    }
    SUPER()
#undef GO
...
static void* findDBusNewConnectionFunctionFct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_DBusNewConnectionFunction_fct_##A == (uintptr_t)fct) return my_DBusNewConnectionFunction_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_DBusNewConnectionFunction_fct_##A == 0) {           \
        my_DBusNewConnectionFunction_fct_##A = (uintptr_t)fct; \
        return my_DBusNewConnectionFunction_##A;               \
    }
    SUPER()
#undef GO
