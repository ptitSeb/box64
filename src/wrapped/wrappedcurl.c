...
static void* find_seek_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_seek_fct_##A == (uintptr_t)fct) return my_seek_##A;
    SUPER()
#undef GO
#define GO(A)                             \
    if (my_seek_fct_##A == 0) {           \
        my_seek_fct_##A = (uintptr_t)fct; \
        return my_seek_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_header_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_header_fct_##A == (uintptr_t)fct) return my_header_##A;
    SUPER()
#undef GO
#define GO(A)                               \
    if (my_header_fct_##A == 0) {           \
        my_header_fct_##A = (uintptr_t)fct; \
        return my_header_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_progress_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_progress_fct_##A == (uintptr_t)fct) return my_progress_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_progress_fct_##A == 0) {           \
        my_progress_fct_##A = (uintptr_t)fct; \
        return my_progress_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_progress_int_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_progress_int_fct_##A == (uintptr_t)fct) return my_progress_int_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_progress_int_fct_##A == 0) {           \
        my_progress_int_fct_##A = (uintptr_t)fct; \
        return my_progress_int_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_socket_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_socket_fct_##A == (uintptr_t)fct) return my_socket_##A;
    SUPER()
#undef GO
#define GO(A)                               \
    if (my_socket_fct_##A == 0) {           \
        my_socket_fct_##A = (uintptr_t)fct; \
        return my_socket_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_timer_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_timer_fct_##A == (uintptr_t)fct) return my_timer_##A;
    SUPER()
#undef GO
#define GO(A)                              \
    if (my_timer_fct_##A == 0) {           \
        my_timer_fct_##A = (uintptr_t)fct; \
        return my_timer_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_push_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_push_fct_##A == (uintptr_t)fct) return my_push_##A;
    SUPER()
#undef GO
#define GO(A)                             \
    if (my_push_fct_##A == 0) {           \
        my_push_fct_##A = (uintptr_t)fct; \
        return my_push_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_debug_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_debug_fct_##A == (uintptr_t)fct) return my_debug_##A;
    SUPER()
#undef GO
#define GO(A)                              \
    if (my_debug_fct_##A == 0) {           \
        my_debug_fct_##A = (uintptr_t)fct; \
        return my_debug_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_lockcb_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_lockcb_fct_##A == (uintptr_t)fct) return my_lockcb_##A;
    SUPER()
#undef GO
#define GO(A)                               \
    if (my_lockcb_fct_##A == 0) {           \
        my_lockcb_fct_##A = (uintptr_t)fct; \
        return my_lockcb_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_unlockcb_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_unlockcb_fct_##A == (uintptr_t)fct) return my_unlockcb_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_unlockcb_fct_##A == 0) {           \
        my_unlockcb_fct_##A = (uintptr_t)fct; \
        return my_unlockcb_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_ssl_ctx_callback_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_ssl_ctx_callback_fct_##A == (uintptr_t)fct) return my_ssl_ctx_callback_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_ssl_ctx_callback_fct_##A == 0) {           \
        my_ssl_ctx_callback_fct_##A = (uintptr_t)fct; \
        return my_ssl_ctx_callback_##A;               \
    }
    SUPER()
#undef GO
