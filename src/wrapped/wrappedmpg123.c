...
#define GO(A) \
    if (my_r_read_fct_##A == (uintptr_t)fct) return my_r_read_##A;
    SUPER()
#undef GO
#define GO(A)                               \
    if (my_r_read_fct_##A == 0) {           \
        my_r_read_fct_##A = (uintptr_t)fct; \
        return my_r_read_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_r_lseek_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_r_lseek_fct_##A == (uintptr_t)fct) return my_r_lseek_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_r_lseek_fct_##A == 0) {           \
        my_r_lseek_fct_##A = (uintptr_t)fct; \
        return my_r_lseek_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_cleanup_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_cleanup_fct_##A == (uintptr_t)fct) return my_cleanup_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_cleanup_fct_##A == 0) {           \
        my_cleanup_fct_##A = (uintptr_t)fct; \
        return my_cleanup_##A;               \
    }
    SUPER()
#undef GO
