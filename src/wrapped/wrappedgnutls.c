...
#define GO(A) \
    if (my_gnutls_log_fct_##A == (uintptr_t)fct) return my_gnutls_log_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_gnutls_log_fct_##A == 0) {           \
        my_gnutls_log_fct_##A = (uintptr_t)fct; \
        return my_gnutls_log_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_pullpush_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_pullpush_fct_##A == (uintptr_t)fct) return my_pullpush_##A;
    SUPER()
#undef GO
#define GO(A)                                 \
    if (my_pullpush_fct_##A == 0) {           \
        my_pullpush_fct_##A = (uintptr_t)fct; \
        return my_pullpush_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_timeout_Fct(void* fct)
{
    ...
    if ((p = GetNativeFnc((uintptr_t)fct))) return p;
#define GO(A) \
    if (my_timeout_fct_##A == (uintptr_t)fct) return my_timeout_##A;
    SUPER()
#undef GO
#define GO(A)                                \
    if (my_timeout_fct_##A == 0) {           \
        my_timeout_fct_##A = (uintptr_t)fct; \
        return my_timeout_##A;               \
    }
    SUPER()
#undef GO
