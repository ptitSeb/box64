...
#define GO(A) \
    if (my_FAudioMalloc_fct_##A == (uintptr_t)fct) return my_FAudioMalloc_##A;
    SUPER()
#undef GO
#define GO(A)                                     \
    if (my_FAudioMalloc_fct_##A == 0) {           \
        my_FAudioMalloc_fct_##A = (uintptr_t)fct; \
        return my_FAudioMalloc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FAudioFree_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_FAudioFree_fct_##A == (uintptr_t)fct) return my_FAudioFree_##A;
    SUPER()
#undef GO
#define GO(A)                                   \
    if (my_FAudioFree_fct_##A == 0) {           \
        my_FAudioFree_fct_##A = (uintptr_t)fct; \
        return my_FAudioFree_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FAudioRealloc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_FAudioRealloc_fct_##A == (uintptr_t)fct) return my_FAudioRealloc_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_FAudioRealloc_fct_##A == 0) {           \
        my_FAudioRealloc_fct_##A = (uintptr_t)fct; \
        return my_FAudioRealloc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnCriticalErrorFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnCriticalErrorFunc_fct_##A == (uintptr_t)fct) return my_OnCriticalErrorFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                            \
    if (my_OnCriticalErrorFunc_fct_##A == 0) {           \
        my_OnCriticalErrorFunc_fct_##A = (uintptr_t)fct; \
        return my_OnCriticalErrorFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnProcessingPassEndFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnProcessingPassEndFunc_fct_##A == (uintptr_t)fct) return my_OnProcessingPassEndFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                \
    if (my_OnProcessingPassEndFunc_fct_##A == 0) {           \
        my_OnProcessingPassEndFunc_fct_##A = (uintptr_t)fct; \
        return my_OnProcessingPassEndFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnProcessingPassStartFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnProcessingPassStartFunc_fct_##A == (uintptr_t)fct) return my_OnProcessingPassStartFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                  \
    if (my_OnProcessingPassStartFunc_fct_##A == 0) {           \
        my_OnProcessingPassStartFunc_fct_##A = (uintptr_t)fct; \
        return my_OnProcessingPassStartFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnBufferEndFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnBufferEndFunc_fct_##A == (uintptr_t)fct) return my_OnBufferEndFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_OnBufferEndFunc_fct_##A == 0) {           \
        my_OnBufferEndFunc_fct_##A = (uintptr_t)fct; \
        return my_OnBufferEndFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnBufferStartFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnBufferStartFunc_fct_##A == (uintptr_t)fct) return my_OnBufferStartFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                          \
    if (my_OnBufferStartFunc_fct_##A == 0) {           \
        my_OnBufferStartFunc_fct_##A = (uintptr_t)fct; \
        return my_OnBufferStartFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnLoopEndFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnLoopEndFunc_fct_##A == (uintptr_t)fct) return my_OnLoopEndFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                      \
    if (my_OnLoopEndFunc_fct_##A == 0) {           \
        my_OnLoopEndFunc_fct_##A = (uintptr_t)fct; \
        return my_OnLoopEndFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnStreamEndFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnStreamEndFunc_fct_##A == (uintptr_t)fct) return my_OnStreamEndFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                        \
    if (my_OnStreamEndFunc_fct_##A == 0) {           \
        my_OnStreamEndFunc_fct_##A = (uintptr_t)fct; \
        return my_OnStreamEndFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnVoiceErrorFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnVoiceErrorFunc_fct_##A == (uintptr_t)fct) return my_OnVoiceErrorFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                         \
    if (my_OnVoiceErrorFunc_fct_##A == 0) {           \
        my_OnVoiceErrorFunc_fct_##A = (uintptr_t)fct; \
        return my_OnVoiceErrorFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnVoiceProcessingPassEndFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnVoiceProcessingPassEndFunc_fct_##A == (uintptr_t)fct) return my_OnVoiceProcessingPassEndFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                     \
    if (my_OnVoiceProcessingPassEndFunc_fct_##A == 0) {           \
        my_OnVoiceProcessingPassEndFunc_fct_##A = (uintptr_t)fct; \
        return my_OnVoiceProcessingPassEndFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_OnVoiceProcessingPassStartFunc_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_OnVoiceProcessingPassStartFunc_fct_##A == (uintptr_t)fct) return my_OnVoiceProcessingPassStartFunc_##A;
    SUPER()
#undef GO
#define GO(A)                                                       \
    if (my_OnVoiceProcessingPassStartFunc_fct_##A == 0) {           \
        my_OnVoiceProcessingPassStartFunc_fct_##A = (uintptr_t)fct; \
        return my_OnVoiceProcessingPassStartFunc_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FAudioEngineCallEXT_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_FAudioEngineCallEXT_fct_##A == (uintptr_t)fct) return my_FAudioEngineCallEXT_##A;
    SUPER()
#undef GO
#define GO(A)                                            \
    if (my_FAudioEngineCallEXT_fct_##A == 0) {           \
        my_FAudioEngineCallEXT_fct_##A = (uintptr_t)fct; \
        return my_FAudioEngineCallEXT_##A;               \
    }
    SUPER()
#undef GO
...
static void* find_FAudioEngineProcedureEXT_Fct(void* fct)
{
    ...
    if (GetNativeFnc((uintptr_t)fct)) return GetNativeFnc((uintptr_t)fct);
#define GO(A) \
    if (my_FAudioEngineProcedureEXT_fct_##A == (uintptr_t)fct) return my_FAudioEngineProcedureEXT_##A;
    SUPER()
#undef GO
#define GO(A)                                                 \
    if (my_FAudioEngineProcedureEXT_fct_##A == 0) {           \
        my_FAudioEngineProcedureEXT_fct_##A = (uintptr_t)fct; \
        return my_FAudioEngineProcedureEXT_##A;               \
    }
    SUPER()
#undef GO
