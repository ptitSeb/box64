...
extern void ResetTable64(dynarec_native_t* dyn);

void updateflags_pass0(dynarec_ppc64le_t* dyn, uint64_t jmp_df[]);
void updateflags_pass1(dynarec_ppc64le_t* dyn, uint64_t jmp_df[]);
void updateflags_pass2(dynarec_ppc64le_t* dyn, uint64_t jmp_df[]);
void updateflags_pass3(dynarec_ppc64le_t* dyn, uint64_t jmp_df[]);

static dynablock_t* updaflags_ppc64le = NULL;

static uint8_t dummy_code[] = {0x90, 0xc3}; // some dummy code so update_flags dynablock point to something

void* create_updateflags()
{
    if(updaflags_ppc64le)
        return updaflags_ppc64le->block;
    uint64_t jmp_df[d_unknown+1] = {0};
    dynarec_ppc64le_t helper = {0};
    instruction_ppc64le_t insts[1] = {0};
    helper.insts = insts;
    ...
    //           dynablock_t*     block (ppc insts)            table64               jmpnext code instsize arch callrets     dynablock      relocs
