...
const char* arm64_print(uint32_t opcode, uintptr_t addr)
{
    ...
    if(isMask(opcode, "f1001011001mmmmmoooiiinnnnnddddd", &a)) {
        ...
        if(((sf==0 && option==2) || (sf==1 && option==3)) && imm==0)
            snprintf(buff, sizeof(buff), "SUB %s, %s, %s", sf?XtSp[Rd]:WtSp[Rd], sf?XtSp[Rn]:WtSp[Rn], sf?XtSp[Rm]:WtSp[Rm]);
        else if(imm==0)
            snprintf(buff, sizeof(buff), "SUB %s, %s, %s %s", sf?Xt[Rd]:Wt[Rd], sf?Xt[Rn]:Wt[Rn], sf?Xt[Rm]:Wt[Rm], shifts[sf][option]);
        else
            snprintf(buff, sizeof(buff), "SUB %s, %s, %s %s %d", sf?Xt[Rd]:Wt[Rd], sf?Xt[Rn]:Wt[Rn], sf?Xt[Rm]:Wt[Rm], shifts[sf][option], imm);
        return buff;
    }
    if(isMask(opcode, "f0001011001mmmmmoooiiinnnnnddddd", &a)) {
        ...
        if(((sf==0 && option==2) || (sf==1 && option==3)) && imm==0)
            ...
            snprintf(buff, sizeof(buff), "ADD %s, %s, %s %s", sf?Xt[Rd]:Wt[Rd], sf?Xt[Rn]:Wt[Rn], sf?Xt[Rm]:Wt[Rm], shifts[sf][option]);
        else
            snprintf(buff, sizeof(buff), "ADD %s, %s, %s %s %d", sf?Xt[Rd]:Wt[Rd], sf?Xt[Rn]:Wt[Rn], sf?Xt[Rm]:Wt[Rm], shifts[sf][option], imm);
        return buff;
    }
    // ---- LOGIC
