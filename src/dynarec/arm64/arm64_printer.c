...
#define opc    a.c
    ...
    if (isMask(opcode, "0Q001110000rrrrr001111nnnnnddddd", &a)) {
        ...
        if (a.Q == 0 && immr & 1) {
            s = 'B';
            sz = 0;
        } else if (a.Q == 0 && (immr & 3) == 2) {
            s = 'H';
            sz = 1;
        } else if (a.Q == 0 && (immr & 7) == 4) {
            s = 'S';
            sz = 2;
        } else if (a.Q == 1 && (immr & 15) == 8) {
            s = 'D';
            sz = 3;
        }
        int index = (immr) >> (sz + 1);
    ...
    if (isMask(opcode, "0QU011110hhhhrrr000001nnnnnddddd", &a)) {
        ...
        if (shift == 0b0001) {
            Vd = Y[a.Q];
            s = 16 - ((shift) << 3 | immr);
        } else if ((shift & 0b1110) == 0b0010) {
            Vd = Y[2 + a.Q];
            s = 32 - ((shift) << 3 | immr);
        } else if ((shift & 0b1100) == 0b0100) {
            Vd = Y[4 + a.Q];
            s = 64 - ((shift) << 3 | immr);
        } else if ((shift & 0b1000) == 0b1000) {
            Vd = Y[6 + a.Q];
            s = 128 - ((shift) << 3 | immr);
        }
    ...
    if (isMask(opcode, "f0011110pp10x11c000000nnnnnddddd", &a)) {
        ...
        int rmode = a.x;
        int opcd = 6 + a.c;
        if (sf == 0 && type == 0 && rmode == 0 && opcd == 7) {
            snprintf(buff, sizeof(buff), "FMOV S%d, %s", Rd, Wt[Rn]);
        } else if (sf == 0 && type == 0 && rmode == 0 && opcd == 6) {
            snprintf(buff, sizeof(buff), "FMOV %s, S%d", Wt[Rn], Rd);
        } else if (sf == 1 && type == 1 && rmode == 0 && opcd == 7) {
            snprintf(buff, sizeof(buff), "FMOV D%d, %s", Rd, Xt[Rn]);
        } else if (sf == 1 && type == 2 && rmode == 1 && opcd == 7) {
            snprintf(buff, sizeof(buff), "FMOV V%d.D[1], %s", Rd, Xt[Rn]);
        } else if (sf == 1 && type == 1 && rmode == 0 && opcd == 6) {
            snprintf(buff, sizeof(buff), "FMOV %s, D%d", Xt[Rd], Rn);
        } else if (sf == 1 && type == 2 && rmode == 1 && opcd == 6) {
            snprintf(buff, sizeof(buff), "FMOV %s, V%d.D[1]", Xt[Rd], Rn);
        } else
            snprintf(buff, sizeof(buff), "FMOV ????");
        return buff;
    }
    ...
    if (isMask(opcode, "0QU011110hhhhiii101001nnnnnddddd", &a)) {
        ...
        const char* Z[] = { "8H", "4S", "2D", "?" };
        int sz = 3;
        if ((a.h & 0b1111) == 0b0001)
            sz = 0;
        else if ((a.h & 0b1110) == 0b0010)
            sz = 1;
        else if ((a.h & 0b1100) == 0b0100)
            sz = 2;
    ...
    if (isMask(opcode, "0Q001110000iiiii000001nnnnnddddd", &a)) {
        ...
        int sz = 3;
        if ((imm & 0b0001) == 0b0001)
            sz = 0;
        else if ((imm & 0b0011) == 0b0010)
            sz = 1;
        else if ((imm & 0b0111) == 0b0100)
            sz = 2;
        int sh = imm - (1 << sz);
        const char* Vd = Y[(sz << 1) | a.Q];
        const char* Vn = Z[sz];
