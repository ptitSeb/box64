...
#define SMMIGHTREAD()
#define WILLWRITE2() \
    if (box64_dynarec_strongmem > SMWRITE2_MIN) { WILLWRITE(); }
#define SMWRITE2() \
    if (box64_dynarec_strongmem > SMWRITE2_MIN) { SMWRITE(); }
...
#define SMSTART()     \
    ...
    dyn->smread = 0;
#define SMEND()                                                    \
    if (dyn->smwrite && (box64_dynarec_strongmem > SMFIRST_MIN)) { \
        int i = ninst;                                             \
...
// Opcode has wrote
#define SMWRITE()                                                                                                              \
    if ((box64_dynarec_strongmem >= SMFIRST_MIN) && dyn->smwrite == 0 && (box64_dynarec_strongmem != SMREAD_VAL)) { SMDMB(); } \
    if (box64_dynarec_strongmem > SMSEQ_MIN && (box64_dynarec_strongmem != SMREAD_VAL)) {                                      \
        if (++dyn->smwrite >= SMSEQ_MAX) {                                                                                     \
            SMDMB();                                                                                                           \
            dyn->smwrite = 1;                                                                                                  \
        }                                                                                                                      \
    } else                                                                                                                     \
...
#define WILLWRITE()                                                                                                                 \
    if ((box64_dynarec_strongmem >= SMFIRST_MIN) && dyn->smwrite == 0 && (box64_dynarec_strongmem != SMREAD_VAL)) {                 \
        SMDMB();                                                                                                                    \
...
// End of sequence
#define SMEND()                                                                                            \
    if (dyn->smwrite && box64_dynarec_strongmem && (box64_dynarec_strongmem != SMREAD_VAL)) { DMB_ISH(); } \
    dyn->smwrite = 0;                                                                                      \
    dyn->smread = 0
...
#define STORE_XEMU_CALL(A)                                           \
    ...
    STP_REGS(R14, R15);                                              \
    if (A) {                                                         \
        STPx_S7_offset(xFlags, A, xEmu, offsetof(x64emu_t, eflags)); \
    } else {                                                         \
        STRx_U12(xFlags, xEmu, offsetof(x64emu_t, eflags));          \
    }
...
#define LOAD_XEMU_CALL(A)                                            \
    if (A) {                                                         \
        LDPx_S7_offset(xFlags, A, xEmu, offsetof(x64emu_t, eflags)); \
    } else {                                                         \
        LDRx_U12(xFlags, xEmu, offsetof(x64emu_t, eflags));          \
    };                                                               \
...
#endif
#define UFLAG_OP1(A) \
    if (dyn->insts[ninst].x64.gen_flags) { STRxw_U12(A, xEmu, offsetof(x64emu_t, op1)); }
#define UFLAG_OP2(A) \
    if (dyn->insts[ninst].x64.gen_flags) { STRxw_U12(A, xEmu, offsetof(x64emu_t, op2)); }
#define UFLAG_OP12(A1, A2)                            \
    if (dyn->insts[ninst].x64.gen_flags) {            \
        STRxw_U12(A1, xEmu, offsetof(x64emu_t, op1)); \
        STRxw_U12(A2, xEmu, offsetof(x64emu_t, op2)); \
    }
#define UFLAG_RES(A) \
    if (dyn->insts[ninst].x64.gen_flags) { STRxw_U12(A, xEmu, offsetof(x64emu_t, res)); }
#define UFLAG_DF(r, A) \
...
#define GOCOND(B, T1, T2)                                             \
    ...
    case B + 0x4:                                                     \
        INST_NAME(T1 "Z " T2);                                        \
        GO(TSTw_mask(xFlags, 0b011010, 0), cEQ, cNE, X_ZF)            \
        break;                                                        \
    case B + 0x5:                                                     \
        INST_NAME(T1 "NZ " T2);                                       \
        GO(TSTw_mask(xFlags, 0b011010, 0), cNE, cEQ, X_ZF)            \
        break;                                                        \
    case B + 0x6:                                                     \
        INST_NAME(T1 "BE " T2);                                       \
    ...
    case B + 0xC:                                                     \
        ...
        GO(EORw_REG_LSL(x1, xFlags, xFlags, F_OF - F_SF);             \
            TSTw_mask(x1, 0b010101, 0), cEQ, cNE, X_SF | X_OF)        \
        break;                                                        \
    case B + 0xD:                                                     \
        INST_NAME(T1 "GE " T2);                                       \
        GO(EORw_REG_LSL(x1, xFlags, xFlags, F_OF - F_SF);             \
            TSTw_mask(x1, 0b010101, 0), cNE, cEQ, X_SF | X_OF)        \
        break;                                                        \
    case B + 0xE:                                                     \
        INST_NAME(T1 "LE " T2);                                       \
        GO(EORw_REG_LSL(x1, xFlags, xFlags, F_OF - F_SF);             \
            ORRw_REG_LSL(x1, x1, xFlags, F_OF - F_ZF);                \
            TSTw_mask(x1, 0b010101, 0), cEQ, cNE, X_SF | X_OF | X_ZF) \
        break;                                                        \
    case B + 0xF:                                                     \
        INST_NAME(T1 "G " T2);                                        \
        GO(EORw_REG_LSL(x1, xFlags, xFlags, F_OF - F_SF);             \
            ORRw_REG_LSL(x1, x1, xFlags, F_OF - F_ZF);                \
            TSTw_mask(x1, 0b010101, 0), cNE, cEQ, X_SF | X_OF | X_ZF) \
        break
