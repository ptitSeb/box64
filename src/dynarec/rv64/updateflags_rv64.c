...
void updateflags_pass0(dynarec_rv64_t* dyn, uint64_t jmp_df[]);
void updateflags_pass1(dynarec_rv64_t* dyn, uint64_t jmp_df[]);
void updateflags_pass2(dynarec_rv64_t* dyn, uint64_t jmp_df[]);
void updateflags_pass3(dynarec_rv64_t* dyn, uint64_t jmp_df[]);

static dynablock_t* updateflags_rv64 = NULL;
...
{
    if (updateflags_rv64)
        return updateflags_rv64->block;

    uint64_t jmp_df[d_unknown + 1] = { 0 };
    dynarec_rv64_t helper = { 0 };
    instruction_rv64_t insts[1] = { 0 };
    helper.insts = insts;
    ...
    //           dynablock_t*     block (riscv insts)          table64               jmpnext code instsize arch callrets     dynablock      relocs
