...
static inline insn_t insn_cjtype_read(uint16_t data)
{
    ...
    int32_t imm = ((imm5 << 5) | (imm31 << 1) | (imm7 << 7) | (imm6 << 6) | (imm10 << 10) | (imm98 << 8) | (imm4 << 4) | (imm11 << 11));
    imm = (imm << 20) >> 20;
    return (insn_t) {
        .imm = imm,
        .rvc = true,
...
#define PRINT_none()                               \
    snprintf(buff, sizeof(buff), "%s", insn.name); \
    return buff
#define PRINT_rd_rs1()                                                      \
    snprintf(buff, sizeof(buff), "%s\t%s, %s", insn.name, RN(rd), RN(rs1)); \
    return buff
#define PRINT_rd_rs1_rs2()                                                               \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s", insn.name, RN(rd), RN(rs1), RN(rs2)); \
    return buff
#define PRINT_rd_rs1_rs2_rs3()                                                                        \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s, %s", insn.name, RN(rd), RN(rs1), RN(rs2), RN(rs3)); \
    return buff
#define PRINT_rd_rs1_imm()                                                                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s0x%x", insn.name, RN(rd), RN(rs1), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm); \
    return buff
#define PRINT_rd_rs1_immx()                                                                 \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, 0x%x", insn.name, RN(rd), RN(rs1), insn.imm); \
    return buff
#define PRINT_rd_imm_rs1()                                                                                                                                    \
    snprintf(buff, sizeof(buff), "%s\t%s, %s0x%x(%s)", insn.name, RN(rd), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, gpnames[insn.rs1]); \
    return buff
#define PRINT_rs2_imm_rs1()                                                                                                                                    \
    snprintf(buff, sizeof(buff), "%s\t%s, %s0x%x(%s)", insn.name, RN(rs2), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, gpnames[insn.rs1]); \
    return buff
#define PRINT_rd_imm()                                                                                                                 \
    snprintf(buff, sizeof(buff), "%s\t%s, %s0x%x", insn.name, RN(rd), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm); \
    return buff
#define PRINT_rd_imm_rel()                                                                                                                                                        \
    snprintf(buff, sizeof(buff), "%s\t%s, pc%s0x%x # 0x%" PRIx64, insn.name, RN(rd), insn.imm >= 0 ? "+" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, insn.imm + (uint64_t)addr); \
    return buff
#define PRINT_imm_rel()                                                                                                                                               \
    snprintf(buff, sizeof(buff), "%s\tpc%s0x%x # 0x%" PRIx64, insn.name, insn.imm >= 0 ? "+" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, insn.imm + (uint64_t)addr); \
    return buff
#define PRINT_rd_immx()                                                        \
    snprintf(buff, sizeof(buff), "%s\t%s, 0x%x", insn.name, RN(rd), insn.imm); \
    return buff
#define PRINT_rs1_rs2_imm()                                                                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s0x%x", insn.name, RN(rs1), RN(rs2), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm); \
    return buff
#define PRINT_rs1_rs2_imm_rel()                                                                                                                                                                 \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, pc%s0x%x # 0x%" PRIx64, insn.name, RN(rs1), RN(rs2), insn.imm >= 0 ? "+" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, insn.imm + (uint64_t)addr); \
    return buff
#define PRINT_fd_fs1()                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %s", insn.name, fpnames[insn.rd], fpnames[insn.rs1]); \
    return buff
#define PRINT_xd_fs1()                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %s", insn.name, gpnames[insn.rd], fpnames[insn.rs1]); \
    return buff
#define PRINT_fd_xs1()                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %s", insn.name, fpnames[insn.rd], gpnames[insn.rs1]); \
    return buff
#define PRINT_rd_fs1_fs2()                                                                         \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s", insn.name, gpnames[insn.rd], RN(rs1), RN(rs2)); \
    return buff
#define PRINT_rd_rs1_aqrl()                                                                                                             \
    snprintf(buff, sizeof(buff), "%s%s%s\t%s, (%s)", insn.name, aq ? ".aq" : "", rl ? ".rl" : "", gpnames[insn.rd], gpnames[insn.rs1]); \
    return buff
#define PRINT_rd_rs1_rs2_aqrl()                                                                                                                                \
    snprintf(buff, sizeof(buff), "%s%s%s\t%s, %s, (%s)", insn.name, aq ? ".aq" : "", rl ? ".rl" : "", gpnames[insn.rd], gpnames[insn.rs2], gpnames[insn.rs1]); \
    return buff
#define PRINT_rd_rs1_rs2_imm()                                                                                                                                   \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s, %s0x%x", insn.name, RN(rd), RN(rs1), RN(rs2), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm); \
    return buff
#define PRINT_rd_rs1_imm1_imm2()                                                                                                                                                                                                \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, %s0x%x, %s0x%x", insn.name, RN(rd), RN(rs1), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm, insn.imm2 >= 0 ? "" : "-", insn.imm2 >= 0 ? insn.imm2 : -insn.imm2); \
    return buff
#define PRINT_rd1_rd2_rs1_imm_4()                                                                                                                                     \
    snprintf(buff, sizeof(buff), "%s\t%s, %s, (%s), %s0x%x, 4", insn.name, RN(rd), RN(rs2), RN(rs1), insn.imm >= 0 ? "" : "-", insn.imm >= 0 ? insn.imm : -insn.imm); \
    return buff

// TODO: display csr name
#define PRINT_rd_csr_rs1()                                                                \
    snprintf(buff, sizeof(buff), "%s\t%s, %d, %s", insn.name, RN(rd), insn.csr, RN(rs1)); \
    return buff
#define PRINT_rd_csr_uimm()                                                                          \
    snprintf(buff, sizeof(buff), "%s\t%s, %d, %d", insn.name, RN(rd), insn.csr, (uint32_t)insn.imm); \
    return buff
