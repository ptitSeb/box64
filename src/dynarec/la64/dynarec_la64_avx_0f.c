            ...
            if (!vex.l) {
                ...
                if (vex.v != 0) {
                    ...
                    for (int i = 0; i < (rex.is32bits ? 8 : 16); ++i) {
                        q1 = avx_get_reg(dyn, ninst, x1, i, 1, LSX_AVX_WIDTH_128);

                        if (dyn->lsx.avxcache[i].v != -1 && dyn->lsx.avxcache[i].width == LSX_AVX_WIDTH_256) {
                            XVPERMI_Q(q1, q2, XVPERMIQ_Vjd << 4 | XVPERMIQ_Vdd);
                        } else {
