...
// default rcfile
static const char default_rcfile[] = "[bash]\n"
                                     "BOX64_LOG=0\n"
                                     "\n"
                                     "[deadcells]\n"
...
#define ENTRYINT(NAME, name, minval, maxval, bits)                    \
    if (param->is_##name##_present) {                                 \
        name = param->name;                                           \
        printf_log(LOG_INFO, "Applying %s=%d\n", #NAME, param->name); \
    }
#define ENTRYBOOL(NAME, name) ENTRYINT(NAME, name, 0, 1, 1)
#define CENTRYBOOL(NAME, name)                                        \
    if (param->is_##name##_present) {                                 \
        my_context->name = param->name;                               \
        printf_log(LOG_INFO, "Applying %s=%d\n", #NAME, param->name); \
    }
#define ENTRYINTPOS(NAME, name)                                       \
    if (param->is_##name##_present) {                                 \
...
#define ENTRYADDR(NAME, name)                                          \
    if (param->is_##name##_present) {                                  \
        name = param->name;                                            \
        printf_log(LOG_INFO, "Applying %s=%zd\n", #NAME, param->name); \
    }
#define ENTRYULONG(NAME, name)                                          \
    if (param->is_##name##_present) {                                   \
        name = param->name;                                             \
        printf_log(LOG_INFO, "Applying %s=%lld\n", #NAME, param->name); \
    }
    SUPER()
#undef ENTRYBOOL
#undef CENTRYBOOL
